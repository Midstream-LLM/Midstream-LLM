...
static NSString *gen_sdpa_fwd_dynamic(void) {
    ...
    // RoPE on Q/K.  MLX's default (traditional=false) stores the two members
    // of a rotation in separate halves of the feature dimension.  The
    // upstream ANE example used adjacent pairs, so keep both emitters here.
...
#else
    int half_hd = HD / 2;
    [m appendFormat:@"        tensor<int32, [4]> sp_b0 = const()[name=string(\"sp_b0\"), val=tensor<int32, [4]>([0,0,0,0])];\n"];
    [m appendFormat:@"        tensor<int32, [4]> sp_bh = const()[name=string(\"sp_bh\"), val=tensor<int32, [4]>([0,0,0,%d])];\n", half_hd];
    [m appendFormat:@"        tensor<int32, [4]> sp_sq = const()[name=string(\"sp_sq\"), val=tensor<int32, [4]>([1,%d,%d,%d])];\n", HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<int32, [4]> sp_sqk = const()[name=string(\"sp_sqk\"), val=tensor<int32, [4]>([1,%d,%d,%d])];\n", KV_HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> q_first2 = slice_by_size(x=q,begin=sp_b0,size=sp_sq)[name=string(\"q_first2\")];\n", HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> q_second = slice_by_size(x=q,begin=sp_bh,size=sp_sq)[name=string(\"q_second\")];\n", HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> nq = mul(x=q_second,y=neg1)[name=string(\"nq_split\")];\n", HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> q_rot = concat(axis=rpax,interleave=rpil,values=(nq,q_first2))[name=string(\"q_rot_split\")];\n", HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> qc = mul(x=q,y=rope_cos)[name=string(\"qc_split\")];\n", HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> qrs = mul(x=q_rot,y=rope_sin)[name=string(\"qrs_split\")];\n", HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> q_rope = add(x=qc,y=qrs)[name=string(\"q_rope_split\")];\n", HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> k_first = slice_by_size(x=k,begin=sp_b0,size=sp_sqk)[name=string(\"k_first\")];\n", KV_HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> k_second = slice_by_size(x=k,begin=sp_bh,size=sp_sqk)[name=string(\"k_second\")];\n", KV_HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> nk = mul(x=k_second,y=neg1)[name=string(\"nk_split\")];\n", KV_HEADS, SEQ, half_hd];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> k_rot = concat(axis=rpax,interleave=rpil,values=(nk,k_first))[name=string(\"k_rot_split\")];\n", KV_HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> kc = mul(x=k,y=rope_cos)[name=string(\"kc_split\")];\n", KV_HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> krs = mul(x=k_rot,y=rope_sin)[name=string(\"krs_split\")];\n", KV_HEADS, SEQ, HD];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> k_rope = add(x=kc,y=krs)[name=string(\"k_rope_split\")];\n", KV_HEADS, SEQ, HD];
#endif

    // GQA: tile K,V from KV_HEADS to HEADS
    [m appendString:@"        int32 cax = const()[name=string(\"cax\"), val=int32(1)];\n"];
    [m appendString:@"        bool cid = const()[name=string(\"cid\"), val=bool(false)];\n"];
    [m appendString:@"        bool cti = const()[name=string(\"cti\"), val=bool(true)];\n"];
    // Interleaving repeated tensors produces [k0,k0,k1,k1,...], matching
    // mx.repeat(..., axis=1) and the CPU backward reduction order.
    ...
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> k_tiled = concat(axis=cax,interleave=cti,values=(%@))[name=string(\"ktile\")];\n", HEADS, SEQ, HD, k_vals];
    [m appendFormat:@"        tensor<fp16, [1,%d,%d,%d]> v_tiled = concat(axis=cax,interleave=cti,values=(%@))[name=string(\"vtile\")];\n", HEADS, SEQ, HD, v_vals];
